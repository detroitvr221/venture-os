...
async function handleJobComplete(
  ...
  // Save report HTML to Supabase Storage for download
  let storagePath: string | null = null;
  if (report?.id) {
    const htmlContent = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${(job?.job_type || 'audit').replace(/_/g, ' ')} Report</title><style>body{font-family:system-ui;background:#0a0a0a;color:#e5e5e5;padding:40px;max-width:800px;margin:0 auto}h1,h2,h3{color:#4FC3F7}pre{background:#111;padding:16px;border-radius:8px;overflow-x:auto;font-size:13px}.score{font-size:48px;font-weight:800;text-align:center;margin:24px 0}</style></head><body><h1>${(job?.job_type || 'audit').replace(/_/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase())}</h1>${job?.target_url ? `<p>URL: ${job.target_url}</p>` : ''}${score != null ? `<div class="score">${score}/100</div>` : ''}<h2>Summary</h2><p>${summary}</p><h2>Full Report</h2><pre>${responseText.replace(/</g, '&lt;')}</pre><footer><p style="color:#737373;margin-top:40px">Generated by Northbridge Digital &middot; ${new Date().toLocaleDateString()}</p></footer></body></html>`;
    storagePath = `${orgId}/reports/${report.id}.html`;
    await db.storage
      .from('workspace')
    ...
    await db.from('reports').update({
      ...
      file_name: `${(job?.job_type || 'report').replace(/_/g, '-')}-${report.id.slice(0, 8)}.html`,
